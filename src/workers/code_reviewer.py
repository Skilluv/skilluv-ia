"""Worker code review — consomme skilluv:queue:code_review — Phase 5.2."""
...
logger = get_logger("worker.code_reviewer")


async def process_code_review_job(ctx: dict, raw_message: dict) -> None:
    """Traite un job de code review pédagogique."""
    ...
    try:
        ...
        payload = CodeReviewPayload.model_validate(message.payload)

        from src.services.code_reviewer import review_code

        result_data = await review_code(payload)
        ...
        await publish_notification(
            ...
            "completed",
            {
                ...
                "fragments_bonus": result_data.fragments_bonus,
            },
        )
